...
main <- function (includeTable=FALSE) {
    ...
    if (DEV == 'web') {
        ...
        save (loaded_variables, file="/homes/pagapow/SmartR/Survival/loaded_variables.Rda")
        save (fetch_params, file="/homes/pagapow/SmartR/Survival/fetch_params.Rda")
    }
    if (DEV == 'r') {
    ...
    if (DEV != FALSE) {
        print ("In development for Survival/run.R")
